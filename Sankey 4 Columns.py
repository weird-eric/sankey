...
df3 = df.groupby([column3,column4, colorColumn])[column2_3_count].count().reset_index()
df3.columns = ['source','target','colorNode','value']
...
for x in range(df3length):
    test = str(df3['colorLink'][x])
    test1 = test.rstrip(")")
    test2 = test1 + ', ' + str(linkAlpha) + ')'
    test3 = test2.lstrip("rgb")
    test4 = 'rgba' + test3
    df3['colorLink'][x] = test4

for x in range(df3length):
    df3['colorNode'][x] = nodeColorManual
...
graphtitle = column1 + ' to ' + column2 + ' to ' + column3 + ' to ' + column4
